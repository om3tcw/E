...
if (!this[CHANNEL.name].favicon) {
  this[CHANNEL.name].favicon = $("<link/>")
    ...
    .attr(
      "href",
      "https://raw.githubusercontent.com/om3tcw/r/emotes/emotes/ogey.png"
    )
    .appendTo("head");
}
({
  ...
  modules: {
    settings: {
      active: 1,
      rank: -1,
      url: "https://cdn.jsdelivr.net/gh/om3tcw/r/customsettingsmodal.js",
      done: true,
    },
    playlist: {
      active: 1,
      rank: -1,
      url: "https://cdn.jsdelivr.net/gh/om3tcw/r/playlistenhancement2.js",
      done: true,
    },
    privmsg: {
      active: 1,
      rank: 1,
      url: "https://cdn.jsdelivr.net/gh/om3tcw/r/pmenhancement.js",
      done: true,
    },
    notifier: {
      active: 1,
      rank: -1,
      url: "https://cdn.jsdelivr.net/gh/om3tcw/r@emotes/notifier.js",
      done: true,
    },
    layout: {
      active: 1,
      rank: -1,
      url: "https://cdn.jsdelivr.net/gh/om3tcw/r/layoutoptions.js",
      done: true,
    },
    userlist: {
      active: 1,
...
>>>>>>> 171fa0b (emote button):js/loaderMain.js
  },
    ...
    if (this.state.pos < this.index.length) {
      if (this.modules[currKey].active) {
        if (this.modules[currKey].rank <= CLIENT.rank) {
          ...
          this.state.pos++;
          let cache =
            typeof this.modules[currKey].cache == "undefined"
              ? this.cache
              : this.modules[currKey].cache;
          this.getScript(
            this.modules[currKey].url,
            this.sequencerLoader.bind(this),
            cache
          );
        } else {
...
$(document).ready(function () {
  ...
  $("#nav-collapsible ul:first-child").append(
    "<li class='dropdown'><a target='_blank' href='https://holodex.net/home'>HoloDex</a></li>"
  );
  $("#nav-collapsible ul:first-child").append(
    "<li class='dropdown'><a target='_blank' href='https://aggie.io/k9z9w_8z47'>om3tcw Aggie</a></li>"
  );
});
...
/* Tabs */ {
  ...
  var tabList = $('<ul class="nav nav-tabs" role="tablist"></ul>').appendTo(
    tabContainer
  );
  ...
  Callbacks.newPoll = function (data) {
    ...
    if (
      ...
      var badgeTxt = $("#pollsbadge").text();
      var pollCnt =
        $("#pollwrap .well.muted").length + (badgeTxt ? parseInt(badgeTxt) : 0);
  ...
  $(
    '<li role="presentation"><a role="tab" data-toggle="tab" aria-expanded="false" href="#calendarTab">Teamup EN</a></li>'
  ).appendTo(tabList);
  var baseCalendarUrl =
    "https://teamup.com/ksua2ar4zft49pdn7c?view=m&showLogo=0&showSearch=0&showProfileAndInfo=0&showSidepanel=1&disableSidepanel=0&showTitle=0&showViewSelector=1&showMenu=0&weekStartDay=mo&showAgendaHeader=1&showAgendaDetails=0&showYearViewHeader=1";

  $(
    '<div role="tabpanel" class="tab-pane" id="calendarTab2"><iframe width="100%" height="600" frameborder="0" scrolling="auto"></iframe></div>'
  ).appendTo(tabContent);
  $(
    '<li role="presentation"><a role="tab" data-toggle="tab" aria-expanded="false" href="#calendarTab2">Oshi Eyes</a></li>'
  ).appendTo(tabList);
  var baseCalendarUrl2 =
    "https://docs.google.com/forms/d/1oqO8DIIyxuKVPvhXSAmxNCy5zCkS8XQAhEKi8a9BK1g/viewform?";
  ...
  if (!Array.isArray(calendars))
    setOpt(
      ...
      (calendars = [
        {
          src: "d426h89oqa3krrq8cj00kbasgo%40group.calendar.google.com",
          color: "2952A3",
        },
      ])
        ...
        if (!keyHeld) {
          ...
          if (inputBox === document.activeElement) {
            if (inputBox.selectionStart == inputBox.selectionEnd) {
              inputBox.value =
                inputVal.substring(0, selSt) +
                "[sp]" +
                inputVal.substring(selSt, selEnd) +
                "[/sp]" +
                inputVal.substring(selEnd, inputVal.length);
...
(function () {
  $("#plcontrol").append(
    '<input type="button" class="btn btn-sm btn-default" value="🐀" id="replacebutton">'
  );
  $("#plcontrol").append(
    '<input type="button" class="btn btn-sm btn-default" value="🔃" id="refreshbutton">'
  );
  ...
  $("#replacebutton").click(function () {
    ...
    if (newId != "") {
      if (newSource == "YT") {
        document.getElementById("ytapiplayer").src =
          " https://www.youtube.com/embed/" +
          newId +
          "?autohide=1&autoplay=1&controls=1&iv_load_policy=3&rel=0&wmode=opaque&enablejsapi=1&origin=https%3A%2F%2Fom3tcw.com&widgetid=2";
      } else {
        document.getElementById("ytapiplayer").src =
          "https://player.twitch.tv?channel=" +
          newId +
          "&parent=om3tcw.com&referrer=location.host";
      }
    }
  });
...
function createHoverImage(jqChatMessage) {
  jqChatMessage.find("a").bind("mouseenter", function () {
    if (ImageHoverEnable) {
      ...
      $("#messagebuffer div:hover .imageHoverPreview")
        .stop(true, false)
        .slideDown(100);
      $("#messagebuffer div:hover").bind("mouseout", function (event) {
        $(this)
          .children(".imageHoverPreview")
          .stop(true, true)
          .slideUp(100)
          .delay(100)
          .removeAttr("style");
...
`);
  ...
  $(".nav.navbar-nav").append(
    '<li><a id="videotoggylogg" href="javascript:void(0)">A/O</a></li>'
  );
  $("#videotoggylogg").click(function () {
    if ($("#videowrap:visible").length) {
      $("#videowrap").hide();
      $("#chatwrap")
        .removeClass("col-lg-5 col-md-5")
  ...
  $(".nav.navbar-nav").append(
    '<li><a id="togglemotd" href="javascript:void(0)">MOTD</a></li>'
  );
  ...
  $("#main")
    ...
    .children()
    .first()
    .after('<div id="chatdisplayrow" class="row"></div>')
    .next()
    .append($("#userlist,#messagebuffer").removeAttr("style"))
    .after('<div id="chatinputrow" class="row"></div>')
    .next()
    .append($("#emotebtndiv,#chatwrap>form"));

  //mikoboat
  const mikoDing = document.createElement("audio");
  mikoDing.setAttribute(
    "src",
    "https://cdn.jsdelivr.net/gh/om3tcw/r@emotes/soundposts/sounds/om3tcw.ogg"
  );
  mikoDing.loop = true;
  mikoDing.volume = 0.1;
  document.getElementsByClassName("navbar-brand")[0].onmouseenter = () =>
    mikoDing.play();
  document.getElementsByClassName("navbar-brand")[0].onmouseleave = () =>
    mikoDing.pause();
  ...
  userGuide.href =
    "https:///github.com/om3tcw/r/blob/emotes/holopeek/User%20Guide.txt";
  ...
  const options = [
    {
      ...
      func: (self) => {
        ...
        if (checkboxElem && textElem)
          self.css =
            checkboxElem.checked && textElem.value && textElem.value !== ""
              ? `body { background-image: url(${textElem.value}); }`
              : null;
      },
      text: {
        value:
          "https:///raw.githubusercontent.com/om3tcw/r/emotes/holopeek/black.png",
        inputEvent: (self) => {
          document.getElementById(`holopeek_${self.id}`).checked = false;
          self.text.value = document.getElementById(
            `holopeek_${self.id}_text`
          ).value;
        },
      },
    },
    ...
    },
    {
      ...
      func: (self) => {
        ImageHoverEnable = !ImageHoverEnable;
      },
    },
    /* 	{
        id: 'poll_alert',
    ...
    },
    {
        ...
        if (checkboxElem && rangeElem) {
          self.css = checkboxElem.checked
            ? `
                ...
                #videowrap-header {display:none} #chatwrap { width: ${
                  rangeElem.value
                }% !important; }`
            : null;
        }
      ...
      range: {
        ...
        inputEvent: (self) => {
          document.getElementById(`holopeek_${self.id}`).checked = false;
          self.range.value = document.getElementById(
            `holopeek_${self.id}_range`
          ).value;
        },
      },
    },
        ...
        if (checkboxElem && rangeElem) {
          ...
          self.css = checkboxElem.checked
            ? `
                ...
                #messagebuffer { background-color: ${bgColor} !important; }   `
            : null;
        }
      },
      range: {
    ...
    },
    {
      ...
      setupFunc: (self) => {
        ...
        lunaButton.onclick = () => {
          ...
          chatwrap.style.pointerEvents =
            chatwrap.style.pointerEvents === "none" ? "all" : "none";
          chatwrap.style.opacity =
            chatwrap.style.pointerEvents === "none" ? 0.25 : 1;
        };
...
#messagebuffer {padding: 0px}`,
        inputEvent: (self) => {
          document.getElementById(`holopeek_${self.id}`).checked = false;
          self.textarea.value = document.getElementById(
            `holopeek_${self.id}_textarea`
          ).value;
        },
      },
  ...
  function prependMessagesWithMJ(username, checked) {
    ...
    const updateChatInput = () => {
      const offTopicEnabled =
        document.getElementById("holopeek_WatchalongOfftopic").checked ||
  ...
  options.forEach((opt) => {
    ...
    const optFunc = () => {
      ...
      if (document.getElementById(`${optId}_style`))
        document.getElementById(`${optId}_style`).remove();
    ...
    if (opt.textarea) {
      ...
      if (opt.textarea.inputEvent)
        textareaElem.oninput = () => opt.textarea.inputEvent(opt);
    ...
    if (opt.range) {
      ...
      if (opt.range.inputEvent)
        rangeElem.oninput = () => opt.range.inputEvent(opt);
    ...
    if (opt.text) {
      ...
      if (opt.text.inputEvent)
        textElem.oninput = () => opt.text.inputEvent(opt);
  ...
  saveButton.innerHTML =
    'Save<img width="24" height="24" alt="save" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABgAAAAYCAQAAABKfvVzAAAAbUlEQVQ4y2NgGLTAk+Exw38csB6bhkc4lePQAhLGDsIZfmPTAtGAaTZOLfg0gLRguAC/BgaqacANqKuBjaGd4RkQtgNZRGnogPuggzgNT+EantJIA8lOItnTRAUr/uQNgo+Iz0Ag+JjBY9BmfgAjpbf/V5agRgAAAABJRU5ErkJggg==">';
  saveButton.onclick = () =>
    options.forEach((opt) => {
      const valueElem = opt.textarea
        ? "textarea"
      ...
      const value = valueElem
        ? opt[valueElem].value
        : document.getElementById(`holopeek_${opt.id}`).checked
        ? 1
        : 0;
  ...
  resetButton.innerHTML =
    'Reset<img width="24" height="24" alt="save" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABgAAAAYCAQAAABKfvVzAAAAPElEQVQ4y2NgGAJAgeE+w38ovA/k4QH/8UDqaCADkGw+WRqIERvVMNQ1PMKaMB7h1uDB8BhD+WOg6OAGADZZd6fzGEl6AAAAAElFTkSuQmCC">';
  resetButton.onclick = () =>
    options.forEach((opt) => {
...
function nicoprocess(mymsg, myclass) {
  ...
  if (
    mymsg.innerHTML !== null &&
    typeof mymsg.innerHTML === "string" &&
    mymsg.innerHTML.trim().length > 0
  ) {
    let txt = document.createElement("div");
    txt.classList.add("videoText");
    if (myclass && myclass.trim().length > 0) txt.classList.add(myclass);
...
function initializeSoundpost(emote, soundurl, preload = false) {
  if (!soundpostPlaybackState[emote]) {
    ...
    if (preload) {
      soundpostPlaybackState[emote].audio.addEventListener(
        "canplaythrough",
        () => {
...
const emoteMap = {
  ":nyaggernap:":
    "https://raw.githubusercontent.com/puchigire/r/emotes/emotes/nyaggernap.jpg",
  ":yakuless:":
    "https://raw.githubusercontent.com/puchigire/r/emotes/emotes/yakuless.gif",
  ":nightynightnyagger:":
    "https://raw.githubusercontent.com/puchigire/r/emotes/emotes/nightynightnyagger.png",
  ":chinpo:":
    "https://raw.githubusercontent.com/puchigire/r/emotes/emotes/chinpo.png",
  ":sharingiscaring:":
    "https://raw.githubusercontent.com/puchigire/r/emotes/emotes/sharingiscaring.png",
  ":pardner:":
    "https://raw.githubusercontent.com/puchigire/r/emotes/emotes/pardner.png",
  ":nyaggerfed:":
    "https://raw.githubusercontent.com/puchigire/r/emotes/emotes/nyaggerfed.png",
  ":nyaggerfish:":
    "https://raw.githubusercontent.com/puchigire/r/emotes/emotes/nyaggerfish.png",
};
        ...
        if (parentElement) {
          ...
          if (timestampElem) {
            timestampElem.style.backgroundImage =
              "url('https://raw.githubusercontent.com/om3tcw/r/refs/heads/emotes/eyes/nyagger.png')";
          }
        }
    ...
    Object.keys(emoteMap).forEach((emote) => {
      ...
      if (offTopicEnabled) {
        if (mymessage.innerHTML.includes(emote)) {
          mymessage.innerHTML = mymessage.innerHTML.replace(
    ...
    if (soundpostState) {
      ...
      emotes.forEach((emote) => {
        ...
        if (soundpost !== undefined) {
          const preload =
            emoteTitle === ":homuhomu:" || emoteTitle === ":rratate:";
          ...
          if (preload && soundpostPlaybackState[emoteTitle].isPreloaded) {
            ...
            soundpostPlaybackState[emoteTitle].audio.addEventListener(
              "canplaythrough",
              () => {
